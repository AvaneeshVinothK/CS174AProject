...
public class Customer {
    ...
    static void searchItems(List<String[]> cart) {
        try {
            ...
            PreparedStatement ps = Main.emartConn.prepareStatement(sql.toString());
    ...
    static void checkout(List<String[]> cart) {
        ...
        try {
            ...
            // Get most recent discount rules
            ResultSet dr = Main.emartConn.createStatement().executeQuery(
            ...
            // Apply discount based on status — each uses its own rate from DB
            ...
            // Apply shipping — waived if pre-discount subtotal > threshold OR customer is New
            double shipping = 0;
            ...
            System.out.println("Discount:  " + String.format("%.0f", discountPct * 100) + "%");
            ...
            // Generate order number safely — MAX avoids duplicate IDs after deletions
            ResultSet maxRs = Main.emartConn.createStatement().executeQuery(
                "SELECT NVL(MAX(order_number), 0) + 1 FROM CustomerOrder");
            maxRs.next();
            int orderNum = maxRs.getInt(1);

            // Insert order with most recent rule_id
            PreparedStatement orderPs = Main.emartConn.prepareStatement(
                "INSERT INTO CustomerOrder VALUES (?, CURRENT_DATE, ?, ?, " +
            ...
            for (String[] item : cart) {
                PreparedStatement containsPs = Main.emartConn.prepareStatement(
            ...
            PreparedStatement last3Ps = Main.emartConn.prepareStatement(
            ...
            PreparedStatement updateStatus = Main.emartConn.prepareStatement(
    ...
    static void viewPreviousOrders() {
        try {
            ...
            while (rs.next()) {
                ...
                PreparedStatement itemPs = Main.emartConn.prepareStatement(
